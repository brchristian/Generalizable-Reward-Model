...
Usage:
    python checkpoint_automated_upload.py --model MODEL_NAME --repo-prefix YOUR_ORG
"""
...
def main(model_name, repo_prefix):
    # --------------- CONFIG ----------------
    REPO_ID        = f"{repo_prefix}/{model_name}"     # One Hub repo for all checkpoints
    ...
    with TemporaryDirectory(prefix="hf_ckpt_repo_") as tmpdir:
        ...
        for step, ckpt_path in checkpoints:
            ...
            if commit.returncode != 0:
                print(f"Nothing to commit for {ckpt_path.name}; skipping tag.")
        ...
        print(f"  model = AutoModel.from_pretrained('{REPO_ID}')                 # best")
        print(f"  model = AutoModel.from_pretrained('{REPO_ID}', revision='step-{best_step}')")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upload RM checkpoints to HF Hub as tagged revisions")
    parser.add_argument("--model", required=True, help="Model name (matches save_reward_models/<model> directory)")
    parser.add_argument("--repo-prefix", required=True, help="HF Hub organization or user (e.g., 'my-org')")
    args = parser.parse_args()
    try:
        main(args.model, args.repo_prefix)
